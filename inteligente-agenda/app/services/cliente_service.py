...
def listar_horarios_livres(empresa_id: int, profissional_id: int, servico_id: int, data_iso: str):
    ...
    print('listando horarios_livres.')
...
def salvar_agendamento(empresa_id: int, profissional_id: int, servico_id: int, nome_cliente: str, whatsapp_cliente: str, data_hora_iso: str):
    try:
        # 1. Blindagem de tipagem (Garante que IDs sejam números)
        id_empresa_limpo = int(float(empresa_id))
        id_profissional_limpo = int(float(profissional_id))
        id_servico_limpo = int(float(servico_id))

        # 2. Busca duração do serviço para calcular o horário de término
        res_servico = supabase.table("servicos").select("duracao_minutos").eq("id", id_servico_limpo).single().execute()
        
        if not res_servico.data:
            return "AVISO PARA A IA: Serviço não encontrado. Peça para o cliente verificar o serviço escolhido."
            
        duracao = res_servico.data['duracao_minutos']
        inicio = datetime.fromisoformat(data_hora_iso)
        fim = inicio + timedelta(minutes=duracao)

        # 3. Prepara o dicionário de dados
        dados = {
        ...
        # 4. TENTATIVA DE INSERÇÃO (O teste de fogo do Unique Index)
        # Se o horário estiver ocupado (status != cancelado), o banco lança um erro 23505
        supabase.table("agendamentos").insert(dados).execute()

        # 5. SUCESSO: Agora limpamos agendamentos antigos deste cliente
        # Se ele tinha outro horário pendente, agora o novo é o que vale.
        # Usamos .neq() para garantir que não vamos cancelar o que acabamos de criar.
        supabase.table("agendamentos")\
            .update({"status": "cancelado"})\
            .eq("cliente_contato", whatsapp_cliente)\
            .neq("data_hora_inicio", data_hora_iso)\
            .in_("status", ["pendente", "confirmado"])\
            .execute()

        # Retorno amigável para o sucesso
        return f"Tudo certo, {nome_cliente}! Seu agendamento para as {inicio.strftime('%H:%M')} foi confirmado. Até lá!"

    except Exception as e:
        error_msg = str(e)
        
        # 6. TRATAMENTO DE ERRO (Onde o 500 morre e a IA vira humana)
        # Verificamos pelo nome do índice ou pelo código de erro do Postgres (23505)
        if "unique_agendamento_ativo" in error_msg or "23505" in error_msg:
            # Não damos erro 500, devolvemos uma instrução para a IA
            return "AVISO PARA A IA: Este horário já foi preenchido por outro cliente. Informe ao cliente de forma simpática e peça que ele escolha outro horário disponível."
        
        # Log de erro técnico para você debugar no terminal
        print(f"❌ Erro crítico no salvamento: {error_msg}")
        return "AVISO PARA A IA: Ocorreu um erro técnico de conexão. Peça desculpas e peça para tentar de novo."
# --- MOTOR DE IA ---
...
def processar_mensagem_cliente(empresa_id: int, whatsapp_cliente: str, mensagem: str, historico: list = []):
    ...
    Cliente: {whatsapp_cliente} | Data/Hora Atual: {datetime.now().strftime('%d/%m/%Y %H:%M')}.
    ...
    # TOM DE VOZ: {estilo}
    - Se 'Engraçado': Seja resenheiro, use o sotaque de {cidade}. Trate o cliente como um 'parça' da barbearia.
